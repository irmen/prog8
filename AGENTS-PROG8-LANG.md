...
- **Efficient buffer iteration**: To efficiently iterate through a buffer using a pointer, it's better to make a copy of the buffer pointer and increment it directly (using `ptr++` and `@(ptr)`) rather than using an offset variable with `@(buffer + offset)`. This eliminates the need for pointer addition in each iteration. **Exception**: If the offset variable is always a `ubyte` (≤ 255), you can use the offset variable (e.g., `buffer[offset]`) because Prog8 optimizes this case for the 6502.
...
## Standard library
- **to discover what modules and routines are available, FIRST consult docs/source/_static/symboldumps/** - these are **skeleton files** named `skeletons-<target>.txt` (e.g. `skeletons-cx16.txt`) that list ALL modules, subroutines, and builtin functions with their signatures. This is much faster than searching through the library source code.
- If these files are absent in the project, retrieve them online from the Prog8 documentation: https://prog8.readthedocs.io/en/latest/libraries.html#low-fi-variable-and-subroutine-definitions-in-all-available-library-modules (It links to the skeleton file for each of the compilation targets)
