...
internal class FunctionCallAsmGen(private val program: PtProgram, private val asmgen: AsmGen6502Internal) {
    ...
    internal fun optimizeIntArgsViaRegisters(sub: PtSub) =
        (sub.parameters.size==1 && sub.parameters[0].type.isIntegerOrBool)
                || (sub.parameters.size==2 && sub.parameters[0].type.isByteOrBool && sub.parameters[1].type.isByteOrBool)
        ...
        if(sub is PtAsmSub) {
            ...
            if(optimizeIntArgsViaRegisters(sub)) {
                if(sub.parameters.size==1) {
                    val register = if (sub.parameters[0].type.isByteOrBool) RegisterOrPair.A else RegisterOrPair.AY
