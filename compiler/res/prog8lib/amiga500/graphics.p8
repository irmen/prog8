...
graphics {
    ...
    ; ---- constants ----
    const ubyte NEWLOCKS = $0001
    const ubyte CR_NEEDS_NO_CONCEALED_RASTERS = $0001
    const ubyte CR_NEEDS_NO_LAYERBLIT_DAMAGE = $0002
    const ubyte ISLESSX = $0001
    const ubyte ISLESSY = $0002
    const ubyte ISGRTRX = $0004
    const ubyte ISGRTRY = $0008
    const ubyte PRESERVE_COLORS = $0001
    const ubyte AVOID_FLICKER = $0002
    const ubyte IGNORE_MCOMPAT = $0004
    const ubyte COPPER_MOVE = $0000
    const ubyte COPPER_WAIT = $0001
    const ubyte CPRNXTBUF = $0002
    const uword CPR_NT_LOF = $8000
    const uword CPR_NT_SHT = $4000
    const uword CPR_NT_SYS = $2000
    const ubyte EXACT_LINE = $0001
    const ubyte HALF_LINE = $0002
    const uword MODE_640 = $8000
    const ubyte PLNCNTMSK = $7
    const ubyte PLNCNTSHFT = $000c
    const ubyte PF2PRI = $40
    const uword COLORON = $0200
    const uword DBLPF = $400
    const uword HOLDNMODIFY = $800
    const ubyte INTERLACE = $0004
    const ubyte PFA_FINE_SCROLL = $F
    const ubyte PFB_FINE_SCROLL_SHIFT = $0004
    const ubyte PF_FINE_SCROLL_MASK = $F
    const ubyte DIW_HORIZ_POS = $7F
    const uword DIW_VRTCL_POS = $1FF
    const ubyte DIW_VRTCL_POS_SHIFT = $0007
    const ubyte DFTCH_MASK = $FF
    const uword VPOSRLOF = $8000
    const long DTAG_DISP = $80000000
    const long DTAG_DIMS = $80001000
    const long DTAG_MNTR = $80002000
    const long DTAG_NAME = $80003000
    const long DTAG_VEC = $80004000
    const uword DI_AVAIL_NOCHIPS = $0001
    const uword DI_AVAIL_NOMONITOR = $0002
    const uword DI_AVAIL_NOTWITHGENLOCK = $0004
    const long DIPF_IS_LACE = $00000001
    const long DIPF_IS_DUALPF = $00000002
    const long DIPF_IS_PF2PRI = $00000004
    const long DIPF_IS_HAM = $00000008
    const long DIPF_IS_ECS = $00000010
    const long DIPF_IS_AA = $00010000
    const long DIPF_IS_PAL = $00000020
    const long DIPF_IS_SPRITES = $00000040
    const long DIPF_IS_GENLOCK = $00000080
    const long DIPF_IS_WB = $00000100
    const long DIPF_IS_DRAGGABLE = $00000200
    const long DIPF_IS_PANELLED = $00000400
    const long DIPF_IS_BEAMSYNC = $00000800
    const long DIPF_IS_EXTRAHALFBRITE = $00001000
    const long DIPF_IS_SPRITES_ATT = $00002000
    const long DIPF_IS_SPRITES_CHNG_RES = $00004000
    const long DIPF_IS_SPRITES_BORDER = $00008000
    const long DIPF_IS_SCANDBL = $00020000
    const long DIPF_IS_SPRITES_CHNG_BASE = $00040000
    const long DIPF_IS_SPRITES_CHNG_PRI = $00080000
    const long DIPF_IS_DBUFFER = $00100000
    const long DIPF_IS_PROGBEAM = $00200000
    const long DIPF_IS_FOREIGN = $80000000
    const ubyte MCOMPAT_MIXED = $0000
    const ubyte MCOMPAT_SELF = $0001
    const long MCOMPAT_NOBODY = -1
    const ubyte DISPLAYNAMELEN = $0020
    const uword SUSERFLAGS = $00FF
    ...
    const uword ANIMHALF = $0020
    const uword RINGTRIGGER = $0001
    const uword BITSET = $8000
    const ubyte BITCLR = $0000
    const ubyte AGNUS = $0001
    const ubyte DENISE = $0001
    ...
    const ubyte BMA_HEIGHT = $0000
    const ubyte BMA_DEPTH = $0004
    const ubyte BMA_WIDTH = $0008
    const ubyte BMA_FLAGS = $000c
    const long BMATags_Friend = $80000000
    const long BMATags_Depth = $80000001
    const long BMATags_PixelFormat = $80000002
    const long BMATags_Clear = $80000003
    const long BMATags_Displayable = $80000004
    const long BMATags_Private1 = $80000005
    const long BMATags_NoMemory = $80000006
    const long BMATags_NoSprite = $80000007
    const long BMATags_Private2 = $80000008
    const long BMATags_Private3 = $80000009
    const long BMATags_ModeWidth = $8000000a
    const long BMATags_ModeHeight = $8000000b
    const long BMATags_RenderFunc = $8000000c
    const long BMATags_SaveFunc = $8000000d
    const long BMATags_UserData = $8000000e
    const long BMATags_Alignment = $8000000f
    const long BMATags_ConstantBytesPerRow = $80000010
    const long BMATags_UserPrivate = $80000011
    const long BMATags_Private4 = $80000012
    const long BMATags_Private5 = $80000013
    const long BMATags_Private6 = $80000014
    const long BMATags_Private7 = $80000015
    const long BMATags_DisplayID = $80000032
    const long BMATags_BitmapInvisible = $80000037
    const long BMATags_BitmapColors = $80000029
    const long BMATags_BitmapColors32 = $80000043
    const ubyte OWNBLITTERn = $0000
    const ubyte QBOWNERn = $0001
    const ubyte BLITMSG_FAULTn = $0002
    const ubyte BLITMSG_FAULT = $0001
    const ubyte QBOWNER = $0001
    ...
    const long SETCHIPREV_BEST = $ffffffff
    const ubyte BUS_16 = $0000
    const ubyte NML_CAS = $0000
    const ubyte BUS_32 = $0001
    const ubyte DBL_CAS = $0002
    const ubyte NTSCn = $0000
    const ubyte NTSC = $0001
    const ubyte GENLOCn = $0001
    const ubyte GENLOC = $0001
    const ubyte PALn = $0002
    const ubyte PAL = $0001
    const ubyte TODA_SAFEn = $0003
    const ubyte TODA_SAFE = $0001
    const ubyte REALLY_PALn = $0004
    const ubyte REALLY_PAL = $0001
    const ubyte LPEN_SWAP_FRAMESn = $0005
    const ubyte LPEN_SWAP_FRAMES = $0001
    const ubyte GRAPHICS_GFXNODES_I = $0001
    const ubyte SS_GRAPHICS = $02
    ...
    const ubyte LAYERSIMPLE = $0001
    const ubyte LAYERSMART = $0002
    const ubyte LAYERSUPER = $0004
    const ubyte LAYERUPDATING = $10
    const ubyte LAYERBACKDROP = $40
    const ubyte LAYERREFRESH = $80
    const uword LAYERIREFRESH = $200
    const uword LAYERIREFRESH2 = $400
    const uword LAYER_CLIPRECTS_LOST = $100
    const uword LAYERSAVEBACK = $800
    const uword LAYERHIDDEN = $1000
    const ubyte NEWLAYERINFO_CALLED = $0001
    const ubyte LAYERS_NOBACKFILL = $0001
    const ubyte LAYERS_BACKFILL = $0000
    const ubyte LAYER_BACKMOST = $0000
    const ubyte LAYER_FRONTMOST = $0001
    const long MONITOR_ID_MASK = $FFFF1000
    const long DEFAULT_MONITOR_ID = $00000000
    const long NTSC_MONITOR_ID = $00011000
    const long PAL_MONITOR_ID = $00021000
    const long LORES_KEY = $00000000
    const long HIRES_KEY = $00008000
    const long SUPER_KEY = $00008020
    const long HAM_KEY = $00000800
    const long LORESLACE_KEY = $00000004
    const long HIRESLACE_KEY = $00008004
    const long SUPERLACE_KEY = $00008024
    const long HAMLACE_KEY = $00000804
    const long LORESDPF_KEY = $00000400
    const long HIRESDPF_KEY = $00008400
    const long SUPERDPF_KEY = $00008420
    const long LORESLACEDPF_KEY = $00000404
    const long HIRESLACEDPF_KEY = $00008404
    const long SUPERLACEDPF_KEY = $00008424
    const long LORESDPF2_KEY = $00000440
    const long HIRESDPF2_KEY = $00008440
    const long SUPERDPF2_KEY = $00008460
    const long LORESLACEDPF2_KEY = $00000444
    const long HIRESLACEDPF2_KEY = $00008444
    const long SUPERLACEDPF2_KEY = $00008464
    const long EXTRAHALFBRITE_KEY = $00000080
    const long EXTRAHALFBRITELACE_KEY = $00000084
    const long HIRESHAM_KEY = $00008800
    const long SUPERHAM_KEY = $00008820
    const long HIRESEHB_KEY = $00008080
    const long SUPEREHB_KEY = $000080a0
    const long HIRESHAMLACE_KEY = $00008804
    const long SUPERHAMLACE_KEY = $00008824
    const long HIRESEHBLACE_KEY = $00008084
    const long SUPEREHBLACE_KEY = $000080a4
    const long LORESSDBL_KEY = $00000008
    const long LORESHAMSDBL_KEY = $00000808
    const long LORESEHBSDBL_KEY = $00000088
    const long HIRESHAMSDBL_KEY = $00008808
    ...
    const ubyte STANDARD_VIEW_X = $81
    const ubyte STANDARD_VIEW_Y = $2C
    ...
    const ubyte ONE_DOTn = $0001
    const ubyte ONE_DOT = $2
    const ubyte FRST_DOTn = $0000
    const ubyte FRST_DOT = $0001
    const long RPTAG_Font = $80000000
    const long RPTAG_APen = $80000002
    const long RPTAG_BPen = $80000003
    const long RPTAG_DrMd = $80000004
    const long RPTAG_OutLinePen = $80000005
    const long RPTAG_OutlinePen = $80000005
    const long RPTAG_WriteMask = $80000006
    const long RPTAG_MaxPen = $80000007
    const long RPTAG_DrawBounds = $80000008
    const long SPRITEA_Width = $81000000
    const long SPRITEA_XReplication = $81000002
    const long SPRITEA_YReplication = $81000004
    const long SPRITEA_OutputHeight = $81000006
    const long SPRITEA_Attached = $81000008
    const long SPRITEA_OldDataFormat = $8100000a
    const long GSTAG_SPRITE_NUM = $82000020
    const long GSTAG_ATTACHED = $82000022
    const long GSTAG_SOFTSPRITE = $82000024
    const long GSTAG_SCANDOUBLED = $83000000
    ...
    const uword CT_COLORFONT = $0001
    const uword CT_GREYFONT = $0002
    const uword CT_ANTIALIAS = $0004
    const ubyte CTB_MAPCOLOR = 0
    const ubyte CTF_MAPCOLOR = $0001
    const long VTAG_END_CM = $00000000
    const long VTAG_CHROMAKEY_CLR = $80000000
    const long VTAG_CHROMAKEY_SET = $80000001
    const long VTAG_BITPLANEKEY_CLR = $80000002
    const long VTAG_BITPLANEKEY_SET = $80000003
    const long VTAG_BORDERBLANK_CLR = $80000004
    const long VTAG_BORDERBLANK_SET = $80000005
    const long VTAG_BORDERNOTRANS_CLR = $80000006
    const long VTAG_BORDERNOTRANS_SET = $80000007
    const long VTAG_CHROMA_PEN_CLR = $80000008
    const long VTAG_CHROMA_PEN_SET = $80000009
    const long VTAG_CHROMA_PLANE_SET = $8000000A
    const long VTAG_ATTACH_CM_SET = $8000000B
    const long VTAG_NEXTBUF_CM = $8000000C
    const long VTAG_BATCH_CM_CLR = $8000000D
    const long VTAG_BATCH_CM_SET = $8000000E
    const long VTAG_NORMAL_DISP_GET = $8000000F
    const long VTAG_NORMAL_DISP_SET = $80000010
    const long VTAG_COERCE_DISP_GET = $80000011
    const long VTAG_COERCE_DISP_SET = $80000012
    const long VTAG_VIEWPORTEXTRA_GET = $80000013
    const long VTAG_VIEWPORTEXTRA_SET = $80000014
    const long VTAG_CHROMAKEY_GET = $80000015
    const long VTAG_BITPLANEKEY_GET = $80000016
    const long VTAG_BORDERBLANK_GET = $80000017
    const long VTAG_BORDERNOTRANS_GET = $80000018
    const long VTAG_CHROMA_PEN_GET = $80000019
    const long VTAG_CHROMA_PLANE_GET = $8000001A
    const long VTAG_ATTACH_CM_GET = $8000001B
    const long VTAG_BATCH_CM_GET = $8000001C
    const long VTAG_BATCH_ITEMS_GET = $8000001D
    const long VTAG_BATCH_ITEMS_SET = $8000001E
    const long VTAG_BATCH_ITEMS_ADD = $8000001F
    const long VTAG_VPMODEID_GET = $80000020
    const long VTAG_VPMODEID_SET = $80000021
    const long VTAG_VPMODEID_CLR = $80000022
    const long VTAG_USERCLIP_GET = $80000023
    const long VTAG_USERCLIP_SET = $80000024
    const long VTAG_USERCLIP_CLR = $80000025
    const long VTAG_PF1_BASE_GET = $80000026
    const long VTAG_PF2_BASE_GET = $80000027
    const long VTAG_SPEVEN_BASE_GET = $80000028
    const long VTAG_SPODD_BASE_GET = $80000029
    const long VTAG_PF1_BASE_SET = $8000002a
    const long VTAG_PF2_BASE_SET = $8000002b
    const long VTAG_SPEVEN_BASE_SET = $8000002c
    const long VTAG_SPODD_BASE_SET = $8000002d
    const long VTAG_BORDERSPRITE_GET = $8000002e
    const long VTAG_BORDERSPRITE_SET = $8000002f
    const long VTAG_BORDERSPRITE_CLR = $80000030
    const long VTAG_SPRITERESN_SET = $80000031
    const long VTAG_SPRITERESN_GET = $80000032
    const long VTAG_PF1_TO_SPRITEPRI_SET = $80000033
    const long VTAG_PF1_TO_SPRITEPRI_GET = $80000034
    const long VTAG_PF2_TO_SPRITEPRI_SET = $80000035
    const long VTAG_PF2_TO_SPRITEPRI_GET = $80000036
    const long VTAG_IMMEDIATE = $80000037
    const long VTAG_FULLPALETTE_SET = $80000038
    const long VTAG_FULLPALETTE_GET = $80000039
    const long VTAG_FULLPALETTE_CLR = $8000003A
    const long VTAG_DEFSPRITERESN_SET = $8000003B
    const long VTAG_DEFSPRITERESN_GET = $8000003C
    const long VC_IntermediateCLUpdate = $80000080
    const long VC_IntermediateCLUpdate_Query = $80000081
    const long VC_NoColorPaletteLoad = $80000082
    const long VC_NoColorPaletteLoad_Query = $80000083
    const long VC_DUALPF_Disable = $80000084
    const long VC_DUALPF_Disable_Query = $80000085
    const ubyte GENLOCK_VIDEO = $2
    const ubyte V_LACE = $4
    const ubyte V_DOUBLESCAN = $8
    const ubyte V_SUPERHIRES = $20
    const ubyte V_PFBA = $40
    const ubyte V_EXTRA_HALFBRITE = $80
    const uword GENLOCK_AUDIO = $100
    const uword V_DUALPF = $400
    const uword V_HAM = $800
    ...
    const ubyte COLORMAP_TYPE_V1_2 = $00
    const ubyte COLORMAP_TYPE_V1_4 = $01
    const ubyte COLORMAP_TYPE_V39 = $02
    const ubyte COLORMAP_TRANSPARENCY = $01
    const ubyte COLORPLANE_TRANSPARENCY = $02
    const ubyte BORDER_BLANKING = $04
    const ubyte BORDER_NOTRANSPARENCY = $08
    const ubyte VIDEOCONTROL_BATCH = $10
    const ubyte USER_COPPER_CLIP = $20
    const ubyte BORDER_SPRITES = $40
    ...
    const ubyte SPRITERESN_ECS = $0000
    const ubyte SPRITERESN_140NS = $0001
    const ubyte SPRITERESN_70NS = $0002
    const ubyte SPRITERESN_35NS = $0003
    const long SPRITERESN_DEFAULT = -1
    ...
    const long PRECISION_EXACT = -1
    const ubyte PRECISION_IMAGE = $0000
    const ubyte PRECISION_ICON = $0010
    const ubyte PRECISION_GUI = $0020
    const long OBP_Precision = $84000000
    const long OBP_FailIfBad = $84000001
    const ubyte PEN_EXCLUSIVE = $0001
    const ubyte PEN_NO_SETCOLOR = $0002
    ...
    const ubyte VPXB_FREE_ME = $0000
    const ubyte VPXF_FREE_ME = $0001
    const ubyte VPXB_VP_LAST = $0001
    const ubyte VPXF_VP_LAST = $0002
    const ubyte VPXB_STRADDLES_256 = $0004
    const ubyte VPXF_STRADDLES_256 = $0010
    const ubyte VPXB_STRADDLES_512 = $0005
    const ubyte VPXF_STRADDLES_512 = $0020
    const ubyte MVP_OK = $0000
    const ubyte MVP_NO_MEM = $0001
    const ubyte MVP_NO_VPE = $0002
    const ubyte MVP_NO_DSPINS = $0003
    const ubyte MVP_NO_DISPLAY = $0004
    const ubyte MVP_OFF_BOTTOM = $0005
    const ubyte MCOP_OK = $0000
    const ubyte MCOP_NO_MEM = $0001
    const ubyte MCOP_NOP = $0002
