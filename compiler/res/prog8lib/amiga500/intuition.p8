...
intuition {
    ...
    const long CLF_INLIST = $00000001
    ...
    const long GMR_GADGETHIT = $00000004
    const long GMR_NOHELPHIT = $00000000
    ...
    const uword GMR_MEACTIVE = $0000
    const uword GMR_NOREUSE = $0002
    const uword GMR_REUSE = $0004
    const uword GMR_VERIFY = $0008
    const uword GMR_NEXTACTIVE = $0010
    const uword GMR_PREVACTIVE = $0020
    ...
    const uword GMRF_NOREUSE = $0002
    const uword GMRF_REUSE = $0004
    const uword GMRF_VERIFY = $0008
    const uword GMRF_NEXTACTIVE = $0010
    const uword GMRF_PREVACTIVE = $0020
    const ubyte GDOMAIN_MINIMUM = $0000
    const ubyte GDOMAIN_NOMINAL = $0001
    const ubyte GDOMAIN_MAXIMUM = $0002
    const uword GMR_KEYACTIVE = $0010
    const uword GMR_KEYVERIFY = $0020
    const uword ICM_SETLOOP = $402
    const uword ICM_CLEARLOOP = $403
    const uword ICM_CHECKLOOP = $404
    ...
    const ubyte DEPTHIMAGE = $00
    const ubyte ZOOMIMAGE = $01
    const ubyte SIZEIMAGE = $02
    const ubyte CLOSEIMAGE = $03
    const ubyte SDEPTHIMAGE = $05
    const ubyte LEFTIMAGE = $0A
    const ubyte UPIMAGE = $0B
    const ubyte RIGHTIMAGE = $0C
    const ubyte DOWNIMAGE = $0D
    const ubyte CHECKIMAGE = $0E
    const ubyte MXIMAGE = $0F
    const ubyte MENUCHECK = $10
    const ubyte AMIGAKEY = $11
    const ubyte ICONIFYIMAGE = $16
    const ubyte MENUMX = $1B
    const ubyte MENUSUB = $1C
    const ubyte SHIFTKEYIMAGE = $2A
    ...
    const uword MENUENABLED = $0001
    const uword MIDRAWN = $0100
    const uword CHECKIT = $0001
    const uword ITEMTEXT = $0002
    const uword COMMSEQ = $0004
    const uword MENUTOGGLE = $0008
    const uword ITEMENABLED = $0010
    const uword HIGHFLAGS = $00C0
    const uword HIGHIMAGE = $0000
    const uword HIGHCOMP = $0040
    const uword HIGHBOX = $0080
    const uword HIGHNONE = $00C0
    const uword CHECKED = $0100
    const uword ISDRAWN = $1000
    const uword HIGHITEM = $2000
    const uword MENUTOGGLED = $4000
    const uword POINTREL = $0001
    const uword PREDRAWN = $0002
    const uword NOISYREQ = $0004
    const uword SIMPLEREQ = $0010
    const uword USEREQIMAGE = $0020
    const uword NOREQBACKFILL = $0040
    const uword REQOFFWINDOW = $1000
    const uword REQACTIVE = $2000
    const uword SYSREQUEST = $4000
    const uword DEFERREFRESH = $8000
    const uword GFLG_GADGHIGHBITS = $0003
    const uword GFLG_GADGHCOMP = $0000
    const uword GFLG_GADGHBOX = $0001
    const uword GFLG_GADGHIMAGE = $0002
    const uword GFLG_GADGHNONE = $0003
    const uword GFLG_GADGIMAGE = $0004
    const uword GFLG_RELBOTTOM = $0008
    const uword GFLG_RELRIGHT = $0010
    const uword GFLG_RELWIDTH = $0020
    const uword GFLG_RELHEIGHT = $0040
    const uword GFLG_RELSPECIAL = $4000
    const uword GFLG_SELECTED = $0080
    const uword GFLG_DISABLED = $0100
    const uword GFLG_LABELMASK = $3000
    const uword GFLG_LABELITEXT = $0000
    ...
    const uword GACT_RELVERIFY = $0001
    const uword GACT_IMMEDIATE = $0002
    const uword GACT_ENDGADGET = $0004
    const uword GACT_FOLLOWMOUSE = $0008
    const uword GACT_RIGHTBORDER = $0010
    const uword GACT_LEFTBORDER = $0020
    const uword GACT_TOPBORDER = $0040
    const uword GACT_BOTTOMBORDER = $0080
    const uword GACT_BORDERSNIFF = $8000
    const uword GACT_TOGGLESELECT = $0100
    const uword GACT_BOOLEXTEND = $2000
    const uword GACT_STRINGLEFT = $0000
    ...
    const uword GTYP_SYSTYPEMASK = $00F0
    const uword GTYP_SIZING = $0010
    const uword GTYP_WDRAGGING = $0020
    const uword GTYP_SDRAGGING = $0030
    const uword GTYP_WDEPTH = $0040
    const uword GTYP_SDEPTH = $0050
    const uword GTYP_WZOOM = $0060
    const uword GTYP_SUNUSED = $0070
    const uword GTYP_CLOSE = $0080
    const uword GTYP_ICONIFY = $0090
    const uword GTYP_GTYPEMASK = $0007
    const uword GTYP_BOOLGADGET = $0001
    const uword GTYP_GADGET0002 = $0002
    const uword GTYP_PROPGADGET = $0003
    const uword GTYP_STRGADGET = $0004
    const uword GTYP_CUSTOMGADGET = $0005
    const long GMORE_BOUNDS = $00000001
    const long GMORE_GADGETHELP = $00000002
    const long GMORE_SCROLLRASTER = $00000004
    const long GMORE_HIDDEN = $00000010
    const long GMORE_BOOPSIGADGET = $00000400
    const long GMORE_FREEIMAGE = $00000800
    const long GMORE_PARENTHIDDEN = $01000000
    const uword BOOLMASK = $0001
    const uword AUTOKNOB = $0001
    const uword FREEHORIZ = $0002
    const uword FREEVERT = $0004
    const uword PROPBORDERLESS = $0008
    const uword KNOBHIT = $0100
    const uword PROPNEWLOOK = $0010
    const uword SMARTKNOBIMAGE = $0020
    const ubyte KNOBHMIN = $0006
    const ubyte KNOBVMIN = $0004
    const uword MAXBODY = $FFFF
    const uword MAXPOT = $FFFF
    const long IDCMP_SIZEVERIFY = $00000001
    const long IDCMP_NEWSIZE = $00000002
    const long IDCMP_REFRESHWINDOW = $00000004
    const long IDCMP_MOUSEBUTTONS = $00000008
    const long IDCMP_MOUSEMOVE = $00000010
    const long IDCMP_GADGETDOWN = $00000020
    const long IDCMP_GADGETUP = $00000040
    const long IDCMP_REQSET = $00000080
    const long IDCMP_MENUPICK = $00000100
    const long IDCMP_CLOSEWINDOW = $00000200
    const long IDCMP_RAWKEY = $00000400
    const long IDCMP_REQVERIFY = $00000800
    const long IDCMP_REQCLEAR = $00001000
    const long IDCMP_MENUVERIFY = $00002000
    const long IDCMP_NEWPREFS = $00004000
    const long IDCMP_DISKINSERTED = $00008000
    ...
    const uword CWCODE_MOVESIZE = $0000
    const uword CWCODE_DEPTH = $0001
    const uword CWCODE_HIDE = $0002
    const uword CWCODE_SHOW = $0003
    const uword MENUHOT = $0001
    const uword MENUCANCEL = $0002
    const uword MENUWAITING = $0003
    const uword OKABORT = $0004
    const uword WBENCHOPEN = $0001
    const uword WBENCHCLOSE = $0002
    const uword WFLG_SIZEGADGET = $0001
    const uword WFLG_DRAGBAR = $0002
    const uword WFLG_DEPTHGADGET = $0004
    const uword WFLG_CLOSEGADGET = $0008
    const uword WFLG_SIZEBRIGHT = $0010
    const uword WFLG_SIZEBBOTTOM = $0020
    const uword WFLG_REFRESHBITS = $00C0
    const uword WFLG_SMART_REFRESH = $0000
    const uword WFLG_SIMPLE_REFRESH = $0040
    const uword WFLG_SUPER_BITMAP = $0080
    const uword WFLG_OTHER_REFRESH = $00C0
    ...
    const uword NOMENU = $001F
    const uword NOITEM = $003F
    const uword NOSUB = $001F
    ...
    const long RECOVERY_ALERT = $00000000
    ...
    const ubyte CURSORUP = $4C
    const ubyte CURSORLEFT = $4F
    const ubyte CURSORRIGHT = $4E
    const ubyte CURSORDOWN = $4D
    const ubyte KEYCODE_Q = $10
    const ubyte KEYCODE_Z = $31
    const ubyte KEYCODE_X = $32
    const ubyte KEYCODE_V = $34
    const ubyte KEYCODE_B = $35
    const ubyte KEYCODE_N = $36
    const ubyte KEYCODE_M = $37
    const ubyte KEYCODE_LESS = $38
    const ubyte KEYCODE_GREATER = $39
    ...
    const ubyte LACEWB = $01
    const uword SCREEN_DRAG = $4000
    const uword MOUSE_ACCEL = $8000
    const ubyte PARALLEL_PRINTER = $00
    const ubyte SERIAL_PRINTER = $01
    const ubyte BAUD_110 = $00
    const ubyte BAUD_300 = $01
    const ubyte BAUD_1200 = $02
    const ubyte BAUD_2400 = $03
    const ubyte BAUD_4800 = $04
    const ubyte BAUD_9600 = $05
    const ubyte BAUD_19200 = $06
    const ubyte BAUD_MIDI = $07
    const ubyte FANFOLD = $00
    const ubyte SINGLE = $80
    const ubyte PICA = $000
    const uword ELITE = $400
    const uword FINE = $800
    const ubyte DRAFT = $000
    const uword LETTER = $100
    const ubyte SIX_LPI = $000
    const uword EIGHT_LPI = $200
    const ubyte IMAGE_POSITIVE = $00
    const ubyte IMAGE_NEGATIVE = $01
    const ubyte ASPECT_HORIZ = $00
    const ubyte ASPECT_VERT = $01
    const ubyte SHADE_BW = $00
    const ubyte SHADE_GREYSCALE = $01
    const ubyte SHADE_COLOR = $02
    const ubyte US_LETTER = $00
    const ubyte US_LEGAL = $10
    const ubyte N_TRACTOR = $20
    const ubyte W_TRACTOR = $30
    const ubyte CUSTOM = $40
    const ubyte EURO_A0 = $50
    const ubyte EURO_A1 = $60
    const ubyte EURO_A2 = $70
    const ubyte EURO_A3 = $80
    const ubyte EURO_A4 = $90
    const ubyte EURO_A5 = $A0
    const ubyte EURO_A6 = $B0
    const ubyte EURO_A7 = $C0
    const ubyte EURO_A8 = $D0
    const ubyte CUSTOM_NAME = $00
    const ubyte ALPHA_P_101 = $01
    const ubyte BROTHER_15XL = $02
    const ubyte CBM_MPS1000 = $03
    const ubyte DIAB_630 = $04
    const ubyte DIAB_ADV_D25 = $05
    const ubyte DIAB_C_150 = $06
    const ubyte EPSON = $07
    const ubyte EPSON_JX_80 = $08
    const ubyte OKIMATE_20 = $09
    const ubyte QUME_LP_20 = $0A
    const ubyte HP_LASERJET = $0B
    const ubyte HP_LASERJET_PLUS = $0C
    const ubyte SBUF_512 = $00
    const ubyte SBUF_1024 = $01
    const ubyte SBUF_2048 = $02
    const ubyte SBUF_4096 = $03
    const ubyte SBUF_8000 = $04
    const ubyte SBUF_16000 = $05
    const ubyte SREAD_BITS = $F0
    const ubyte SWRITE_BITS = $0F
    const ubyte SSTOP_BITS = $F0
    const ubyte SBUFSIZE_BITS = $0F
    const ubyte SPARITY_BITS = $F0
    const ubyte SHSHAKE_BITS = $0F
    const ubyte SPARITY_NONE = $00
    const ubyte SPARITY_EVEN = $01
    const ubyte SPARITY_ODD = $02
    const ubyte SPARITY_MARK = $03
    const ubyte SPARITY_SPACE = $04
    const ubyte SHSHAKE_XON = $00
    const ubyte SHSHAKE_RTS = $01
    const ubyte SHSHAKE_NONE = $02
    const uword CORRECT_RED = $0001
    const uword CORRECT_GREEN = $0002
    const uword CORRECT_BLUE = $0004
    const uword CENTER_IMAGE = $0008
    const uword IGNORE_DIMENSIONS = $0000
    const uword BOUNDED_DIMENSIONS = $0010
    const uword ABSOLUTE_DIMENSIONS = $0020
    const uword PIXEL_DIMENSIONS = $0040
    const uword MULTIPLY_DIMENSIONS = $0080
    const uword INTEGER_SCALING = $0100
    const uword ORDERED_DITHERING = $0000
    ...
    const long DRIF_NEWLOOK = $00000001
    ...
    const uword SCREENTYPE = $000F
    const uword WBENCHSCREEN = $0001
    const uword PUBLICSCREEN = $0002
    const uword CUSTOMSCREEN = $000F
    const uword SHOWTITLE = $0010
    const uword BEEPING = $0020
    const uword CUSTOMBITMAP = $0040
    const uword SCREENBEHIND = $0080
    ...
    const uword PSNF_PRIVATE = $0001
    const ubyte MAXPUBSCREENNAME = $008b
    const uword SHANGHAI = $0001
    const uword POPPUBSCREEN = $0002
    ...
    const uword EO_NOOP = $0001
    const uword EO_DELBACKWARD = $0002
    const uword EO_DELFORWARD = $0003
    const uword EO_MOVECURSOR = $0004
    const uword EO_ENTER = $0005
    const uword EO_RESET = $0006
    const uword EO_REPLACECHAR = $0007
    const uword EO_INSERTCHAR = $0008
    const uword EO_BADFORMAT = $0009
    const uword EO_BIGCHANGE = $000A
    const uword EO_UNDO = $000B
    const uword EO_CLEAR = $000C
    const uword EO_SPECIAL = $000D
    const ubyte SGM_REPLACE = $0001
    const ubyte SGMB_REPLACE = $0000
    const ubyte SGMF_REPLACE = $0001
    const long SGM_FIXEDFIELD = $00000002
    const ubyte SGMB_FIXEDFIELD = $0001
    const ubyte SGMF_FIXEDFIELD = $0002
    const long SGM_NOFILTER = $00000004
    const ubyte SGMB_NOFILTER = $0002
    const ubyte SGMF_NOFILTER = $0004
    const long SGM_EXITHELP = $00000080
    const ubyte SGMB_EXITHELP = $0007
    const ubyte SGMF_EXITHELP = $80
    const ubyte SGA_USE = $1
    const ubyte SGAB_USE = $0000
    const ubyte SGAF_USE = $1
    const ubyte SGA_END = $2
    const ubyte SGAB_END = $0001
    const ubyte SGAF_END = $2
    const ubyte SGA_BEEP = $4
    const ubyte SGAB_BEEP = $0002
    const ubyte SGAF_BEEP = $4
    const ubyte SGA_REUSE = $8
    const ubyte SGAB_REUSE = $0003
    const ubyte SGAF_REUSE = $8
    const ubyte SGA_REDISPLAY = $10
    const ubyte SGAB_REDISPLAY = $0004
    const ubyte SGAF_REDISPLAY = $10
    const ubyte SGA_NEXTACTIVE = $20
    const ubyte SGAB_NEXTACTIVE = $0005
    const ubyte SGAF_NEXTACTIVE = $20
    const ubyte SGA_PREVACTIVE = $40
    const ubyte SGAB_PREVACTIVE = $0006
    const ubyte SGAF_PREVACTIVE = $40
    const ubyte SGH_KEY = $0001
    const ubyte SGH_CLICK = $0002
    const uword IECLASS_NULL = $00
    const uword IECLASS_RAWKEY = $01
    const uword IECLASS_RAWMOUSE = $02
    const uword IECLASS_EVENT = $03
    const uword IECLASS_POINTERPOS = $04
    const uword IECLASS_TIMER = $06
    const uword IECLASS_GADGETDOWN = $07
    const uword IECLASS_GADGETUP = $08
    const uword IECLASS_REQUESTER = $09
    const uword IECLASS_MENULIST = $0A
    const uword IECLASS_CLOSEWINDOW = $0B
    const uword IECLASS_SIZEWINDOW = $0C
    const uword IECLASS_REFRESHWINDOW = $0D
    const uword IECLASS_NEWPREFS = $0E
    const uword IECLASS_DISKREMOVED = $0F
    const uword IECLASS_DISKINSERTED = $10
    const uword IECLASS_ACTIVEWINDOW = $11
    const uword IECLASS_INACTIVEWINDOW = $12
    const uword IECLASS_NEWPOINTERPOS = $13
    const uword IECLASS_MENUHELP = $14
    const uword IECLASS_CHANGEWINDOW = $15
    const uword IECLASS_MAX = $15
    const uword IESUBCLASS_COMPATIBLE = $00
    const uword IESUBCLASS_PIXEL = $01
    const uword IESUBCLASS_TABLET = $02
    const uword IESUBCLASS_NEWTABLET = $03
    const uword IECODE_UP_PREFIX = $80
    const uword IECODEB_UP_PREFIX = $0007
    const uword IECODE_KEY_CODE_FIRST = $00
    const uword IECODE_KEY_CODE_LAST = $77
    const uword IECODE_COMM_CODE_FIRST = $78
    const uword IECODE_COMM_CODE_LAST = $7F
    const uword IECODE_C0_FIRST = $00
    const uword IECODE_C0_LAST = $1F
    const uword IECODE_ASCII_FIRST = $20
    const uword IECODE_ASCII_LAST = $7E
    const uword IECODE_ASCII_DEL = $7F
    const uword IECODE_C1_FIRST = $80
    const uword IECODE_C1_LAST = $9F
    const uword IECODE_LATIN1_FIRST = $A0
    const uword IECODE_LATIN1_LAST = $FF
    const uword IECODE_LBUTTON = $68
    const uword IECODE_RBUTTON = $69
    const uword IECODE_MBUTTON = $6A
    const uword IECODE_NOBUTTON = $FF
    const uword IECODE_NEWACTIVE = $01
    const uword IECODE_NEWSIZE = $02
    const uword IECODE_REFRESH = $03
    const uword IECODE_REQSET = $01
    const uword IECODE_REQCLEAR = $00
    ...
    const ubyte GADGHIGHBITS = $0003
    const ubyte GADGHCOMP = $0000
    const ubyte GADGHBOX = $0001
    const ubyte GADGHIMAGE = $0002
    const ubyte GADGHNONE = $0003
    const ubyte GADGIMAGE = $0004
    const ubyte GRELBOTTOM = $0008
    const ubyte GRELRIGHT = $0010
    const ubyte GRELWIDTH = $0020
    const ubyte GRELHEIGHT = $0040
    const ubyte SELECTED = $0080
    const uword GADGDISABLED = $0100
    const uword LABELMASK = $3000
    const ubyte LABELITEXT = $0000
    const uword LABELSTRING = $1000
    const uword LABELIMAGE = $2000
    const ubyte RELVERIFY = $0001
    const ubyte GADGIMMEDIATE = $0002
    const ubyte ENDGADGET = $0004
    const ubyte FOLLOWMOUSE = $0008
    const ubyte RIGHTBORDER = $0010
    const ubyte LEFTBORDER = $0020
    const ubyte TOPBORDER = $0040
    const ubyte BOTTOMBORDER = $0080
    const uword BORDERSNIFF = $8000
    const uword TOGGLESELECT = $0100
    const uword BOOLEXTEND = $2000
    const ubyte STRINGLEFT = $0000
    const uword STRINGCENTER = $0200
    const uword STRINGRIGHT = $0400
    const uword LONGINT = $0800
    const uword ALTKEYMAP = $1000
    const uword STRINGEXTEND = $2000
    const uword ACTIVEGADGET = $4000
    const uword GADGETTYPE = $fc00
    const uword SYSGADGET = $8000
    const uword SCRGADGET = $4000
    const uword GZZGADGET = $2000
    const uword REQGADGET = $1000
    const ubyte SIZING = $0010
    const ubyte WDRAGGING = $0020
    const ubyte SDRAGGING = $0030
    const ubyte WUPFRONT = $0040
    const ubyte SUPFRONT = $0050
    const ubyte WDOWNBACK = $0060
    const ubyte SDOWNBACK = $0070
    const ubyte CLOSE = $0080
    const ubyte BOOLGADGET = $0001
    const ubyte GADGET0002 = $0002
    const ubyte PROPGADGET = $0003
    const ubyte STRGADGET = $0004
    const ubyte CUSTOMGADGET = $0005
    const ubyte GTYPEMASK = $0007
    const ubyte SIZEVERIFY = $0001
    const ubyte NEWSIZE = $0002
    const ubyte REFRESHWINDOW = $0004
    const ubyte MOUSEBUTTONS = $0008
    const ubyte MOUSEMOVE = $0010
    const ubyte GADGETDOWN = $0020
    const ubyte GADGETUP = $0040
    const ubyte REQSET = $0080
    const uword MENUPICK = $0100
    const uword CLOSEWINDOW = $0200
    const uword RAWKEY = $0400
    const uword REQVERIFY = $0800
    const uword REQCLEAR = $1000
    const uword MENUVERIFY = $2000
    const uword NEWPREFS = $4000
    const uword DISKINSERTED = $8000
    const long DISKREMOVED = $00010000
    const long WBENCHMESSAGE = $00020000
    const long ACTIVEWINDOW = $00040000
    const long INACTIVEWINDOW = $00080000
    const long DELTAMOVE = $00100000
    const long VANILLAKEY = $00200000
    const long INTUITICKS = $00400000
    const long IDCMPUPDATE = $00800000
    const long MENUHELP = $01000000
    const long CHANGEWINDOW = $02000000
    const long LONELYMESSAGE = $80000000
    const ubyte WINDOWSIZING = $0001
    const ubyte WINDOWDRAG = $0002
    const ubyte WINDOWDEPTH = $0004
    const ubyte WINDOWCLOSE = $0008
    const ubyte SIZEBRIGHT = $0010
    const ubyte SIZEBBOTTOM = $0020
    const ubyte REFRESHBITS = $00c0
    const ubyte SMART_REFRESH = $0000
    const ubyte SIMPLE_REFRESH = $0040
    const ubyte SUPER_BITMAP = $0080
    const ubyte OTHER_REFRESH = $00c0
    const uword BACKDROP = $0100
    const uword REPORTMOUSE = $0200
    const uword GIMMEZEROZERO = $0400
    const uword BORDERLESS = $0800
    const uword ACTIVATE = $1000
    const uword WINDOWACTIVE = $2000
    const uword INREQUEST = $4000
    const uword MENUSTATE = $8000
    const long RMBTRAP = $00010000
    const long NOCAREREFRESH = $00020000
    const long WINDOWREFRESH = $01000000
    const long WBENCHWINDOW = $02000000
    const long WINDOWTICKED = $04000000
    const long NW_EXTENDED = $00040000
    const long VISITOR = $08000000
    const long ZOOMED = $10000000
    const long HASZOOM = $20000000
    const long GA_LEFT = $80030001
    const long GA_RELRIGHT = $80030002
    const long GA_TOP = $80030003
    const long GA_RELBOTTOM = $80030004
