...
private fun setDeferMasks(program: PtProgram, errors: IErrorReporter): Map<PtSub, List<PtDefer>> {
    ...
    for((sub, defers) in defersPerSub) {
        ...
        val deferVariable = PtVariable(maskVarName, DataType.forDt(BaseDataType.UBYTE), ZeropageWish.NOT_IN_ZEROPAGE, null, null, sub.position)
...
private fun integrateDefers(subdefers: Map<PtSub, List<PtDefer>>, program: PtProgram, st: SymbolTable) {
    ...
    for(ret in returnsToAugment) {
        ...
        group.add(pushCall)
        group.add(PtBuiltinFunctionCall(invokeDefersRoutineName, true, false, DataType.forDt(BaseDataType.UNDEFINED), ret.position))
        group.add(newRet)
    ...
    for(sub in subEndsToAugment) {
        ...
        if(defer != null) {
            ...
            sub.add(idx+1, ret)
            val invokedefer = PtBuiltinFunctionCall(invokeDefersRoutineName, true, false, DataType.forDt(BaseDataType.UNDEFINED), sub.position)
