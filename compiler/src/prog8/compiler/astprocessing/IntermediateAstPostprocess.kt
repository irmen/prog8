...
private fun integrateDefers(program: PtProgram, st: SymbolTable) {
    ...
    fun invokedeferbefore(node: PtNode) {
        ...
        if (defer != null) {
            ...
            val invokedefer = PtBuiltinFunctionCall("invoke_defer", true, false, DataType.forDt(BaseDataType.UNDEFINED), node.position)
    ...
    for(ret in returnsToAugment) {
        ...
        group.add(pushCall)
        group.add(PtBuiltinFunctionCall("invoke_defer", true, false, DataType.forDt(BaseDataType.UNDEFINED), ret.position))
        group.add(newRet)
    ...
    for(sub in subEndsToAugment) {
        ...
        if(defer != null) {
            ...
            sub.add(idx+1, ret)
            val invokedefer = PtBuiltinFunctionCall("invoke_defer", true, false, DataType.forDt(BaseDataType.UNDEFINED), sub.position)
