...
internal class AstChecker(private val program: Program,
    ...
    override fun visit(subroutine: Subroutine) {
        ...
        if(varbank!=null) {
            ...
            if (targetSub != null) {
                if (targetSub.parameters.size != 1 || !targetSub.parameters[0].type.isUnsignedByte)
                    err("bank subroutine must have exactly one ubyte parameter (call id)")
                ...
                err("@bank must be a ubyte variable or a subroutine(ubyte callid) returning ubyte")
            }
        }
    ...
    override fun visit(decl: VarDecl) {
        ...
        if(decl.type==VarDeclType.MEMORY) {
            ...
            if (decl.datatype.isPointerArray)
                errors.err("pointer arrays cannot be memory-mapped (codegen limitation)", decl.position)
            else if (decl.datatype.isPointer && decl.origin == VarDeclOrigin.SUBROUTINEPARAM)
