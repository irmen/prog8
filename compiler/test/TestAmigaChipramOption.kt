...
"""
        ...
        errors.errors.size shouldBe 1
        errors.errors[0] shouldContain "amiga_chipram option is only valid on amiga targets"
    }
...
"""
    ...
    test("amiga_chipram option on amiga1200 target is accepted") {
        val src = """
chipper {
    %option amiga_chipram
...
"""
        val result = compileText(Amiga1200Target(), optimize = false, src, outputDir, writeAssembly = false)
        result shouldNotBe null
        val blocks = result!!.compilerAst.allBlocks.toList()
        val chipramBlock = blocks.singleOrNull { "amiga_chipram" in it.options() }
        chipramBlock shouldNotBe null
