...
/**
 * Prefixes all symbol names in the given program's AST with type-specific prefixes
 * (p8b_ for blocks, p8s_ for subroutines, p8v_ for variables, p8c_ for constants,
 *  p8l_ for labels, p8t_ for structs).
 *
 * The AST should have simple (unscoped) names for all named nodes at this point.
 * Identifiers and function calls may already have dotted scoped names.
 * After prefixing, call [makeAllNodenamesScoped] to add scoping to the prefix names.
 *
 * Blocks with `noSymbolPrefixing=true` keep their own names intact,
 * but their references to symbols in prefixed blocks still get prefixed.
 *
 * @return A new [SymbolTable] rebuilt from the now-prefixed AST.
 */
fun prefixSymbols(program: PtProgram, options: CompilationOptions, st: SymbolTable): SymbolTable {
    ...
    fun prefixNamedNode(node: PtNamedNode) {
        when(node) {
            ...
            is PtLabel -> if(!node.name.startsWith(GENERATED_LABEL_PREFIX)) node.name = "p8l_${node.name}"
        ...
        when(node) {
            is PtAsmSub -> {
                ...
                if (address?.varbank != null) {
                    address.varbank = address.varbank!!.prefixIdentifierOrThis(node, st)
                }
            }
            ...
            is PtIdentifier -> {
                ...
                if(node.type.isSplitWordArray(program.memsizer) && (lookupName.endsWith("_lsb") || lookupName.endsWith("_msb"))) {
    ...
    nodesToPrefix.forEach { (parent, index) ->
        when(val node = parent.children[index]) {
            is PtIdentifier -> parent.setChild(index, node.prefixIdentifierOrThis(parent, st))
            is PtFunctionCall ->  throw AssemblyError("PtFunctionCall should be processed in their own list, last")
            is PtVariable -> parent.setChild(index, node.prefixVariableOrThis(parent, st))
    ...
    functionCallsToPrefix.reversed().forEach { (parent, index) ->
        ...
        if(node is PtFunctionCall) {
            val prefixedName = PtIdentifier(node.name, DataType.UNDEFINED, node.position).prefixIdentifierOrThis(parent, st)
...
internal fun typePrefixChar(targetNt: StNodeType): Char {
            ...
            when(elt) {
                ...
                is PtAddressOf -> {
                    if(elt.definingBlock()?.options?.noSymbolPrefixing==true)
                        ...
                        newAddr.add(elt.identifier!!.prefixIdentifierOrThis(newAddr, st))
...
private fun PtIdentifier.prefixIdentifierOrThis(parent: PtNode, st: SymbolTable): PtIdentifier {
