    ...
    private fun optimizeLoopCounters(sub: IRSubroutine): Boolean {
        ...
        while (idx < sub.chunks.size) {
            ...
            for (k in idx + 1 until sub.chunks.size) {
                ...
                val hasIncDec = c.instructions.any { it.opcode in setOf(Opcode.INCM, Opcode.DECM) && it.labelSymbol == loopvar }
                ...
                if (k + 1 < sub.chunks.size) {
                    ...
                    if (nxt is IRCodeChunk && nxt.instructions.size == 3) {
                        ...
                        if (i1.opcode == Opcode.LOADM && i1.labelSymbol == loopvar && i1.reg1 != null &&
                            i2.opcode == Opcode.CMP && (i2.reg1 == i1.reg1 || i2.reg2 == i1.reg1) &&
                            i3.opcode == Opcode.BSTNE && i3.labelSymbol == loopLabel) {
            ...
            for (i in incChunk.instructions.indices) {
                ...
                if (ins.labelSymbol == loopvar && ins.opcode in setOf(Opcode.INCM, Opcode.DECM)) {
                    val newOpcode = if (ins.opcode == Opcode.INCM) Opcode.INC else Opcode.DEC
                    incChunk.instructions[i] = IRInstruction(newOpcode, loopType, reg1 = loopReg)
                    break
                }
            }
            ...
            if (newCmpIdx != null) {
                ...
                for (i in cmpChunk.instructions.indices) {
                    ...
                    if (ins.opcode == Opcode.LOADM && ins.labelSymbol == loopvar) {
                        val dest = ins.reg1!!
                        if (dest == loopReg) {
                            cmpChunk.instructions[i] = IRInstruction(Opcode.NOP)
                        } else {
                            val tp = ins.type ?: loopType
                            cmpChunk.instructions[i] = IRInstruction(Opcode.LOADR, tp, reg1 = dest, reg2 = loopReg)
                        }
                        break
                    }
                }
