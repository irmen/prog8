...
Variable initialization
    ...
    The library must therefore have a ``start()`` entrypoint that must be called
    **before any other library routine** can be used.  This is described in detail in
    the :ref:`Library initialization <library_init>` section below.
...
^^^^^^^^^^^^^^^^^^^^^^
.. index:: single: Libraries; Initialization

A library blob **must** have a ``start()`` subroutine.  When the entrypoint is called,
the Prog8 runtime automatically clears the library's BSS section and initializes
variables before ``start()`` itself runs.  Even an empty ``start()`` is sufficient --
the BSS clearing happens as part of the startup code generated by the compiler.
Without calling this entrypoint, all library variables and BSS data contain garbage
and the library code will not work correctly.

The compiler automatically inserts a ``JMP`` instruction to the ``start()`` subroutine
at the very first address of the library (its entrypoint).  For a library loaded at ``$A000``,
this means ``JSR $A000`` runs the initialization and ``$A000`` is the init routine's call address.

After loading the library into memory, **you must call the init routine at address 0** (e.g. ``$A000``)
before calling any other library routine.  This call can be a plain ``JSR`` because it happens
while the library's memory is already mapped at its load address.

From Prog8, declare it as a plain ``extsub`` (not ``@bank``)::
...
See the examples in the next section for complete usage patterns.

The following example library source shows a minimal ``start()``::
...
in this table!
As described in the previous section, the compiler will already have placed a ``JMP`` to the
``start()`` subroutine at address 0.  The jump table entries start immediately after that,
so the first jump table entry is at address +3 (e.g. ``$A003``).
Because the compiler places the jumptable right after the startup JMP instruction,
it looks as though the init call is just the first entry in the jump table --
