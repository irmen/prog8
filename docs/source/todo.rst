...
- Won't do soon: when implementing unsigned longs: remove the unsigned-long constant "hack" that lets unsigned long constants be used as values for signed longs without needing a cast (the ``Antlr2KotlinVisitor.kt:508``/``:548`` "TODO hack" comments, ``AstExpressions.kt:145`` "TODO should be ULONG, but we don't have that type yet", and ``AsmAssignment.kt:129`` ``TODO("unsigned long $pos")``)
...
- ``IRInlineAsmChunk`` with ``isIR=true`` (IR-form inline asm) is not executable: ``VmProgramLoader.kt`` only converts the trivial ``"clc"``/``"sec"`` chunks and throws on everything else. This is the designed-for home of IR-form asm, since ``AstChecker`` restricts ``asm { %ir }`` to the virtual target only; the VM should parse and run the IR instruction lines contained in such chunks. Raw (non-IR) inline asm and ``IRAsmSubroutine`` should NOT be made executable (the VM has no 6502/68k CPU): instead, reject non-IR ``asm`` blocks and ``asmsub`` with a frontend error on the virtual target (currently they pass ``AstChecker`` and only crash at VM load). ``IRInlineBinaryChunk`` (``%binary`` include) stays deferred: the program is not loaded as data, so an embedded blob has no place.
- VM label address loading: largely already implemented. Code/subroutine labels get artificial addresses in ``pass2replaceLabelsByProgIndex`` and ``JUMPI`` follows them (``VirtualMachine.kt``). Remaining gaps: initializers referencing a non-variable label (struct-instance fields, array elements) still throw ``"vm cannot yet load a label address as a value"``, because artificial addresses are only assigned after ``varsToMemory`` runs. Fix: assign artificial addresses in the allocation phase so there is one name-to-address map available everywhere. ``CALLI`` intentionally stays unsupported (artificial addresses are not real memory).

Symbol naming in new codegen backends (not a VM issue)
- ``prefixScopedName`` (``codeGenIntermediate/src/prog8/codegen/intermediate/SymbolPrefixer.kt:206``) hardcodes ``p8s_`` for all middle path parts of a dotted scoped name. Wrong for structs in the path: ``main.MyStruct.field`` produces ``p8s_MyStruct`` (subroutine prefix) instead of ``p8t_MyStruct`` (struct prefix). Fix: look up each middle part in the symbol table and apply ``typePrefixChar()`` per part. Only affects the new6502/m68k generated labels; the VM never sees prefixed names because the virtual target disables prefixing. Pre-existing bug carried over from the 6502 new6502codegen (``AsmGen.kt``).
