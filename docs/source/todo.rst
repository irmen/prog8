...
- fix limitation: 2D array dimensions must be literal numbers at parse time: ``Antlr2KotlinVisitor.kt`` only accepts ``NumericLiteral`` for the ``[rows][cols]`` dimensions and rejects everything else with "2D array dimensions must be constant expressions". Constant identifiers are not folded until ``processAst()`` (``ConstantIdentifierReplacer``), so even ``const COLS = 4; ubyte[3][COLS] matrix`` fails. 1D arrays in contrast keep the dimension as a general expression and fold it later. Fixing this means deferring the dimension check/fold to the constant-folding phase. When fixed, the user docs may need updating too: ``variables.rst`` (2D Arrays section) doesn't currently mention that dimensions must be literal numbers.
