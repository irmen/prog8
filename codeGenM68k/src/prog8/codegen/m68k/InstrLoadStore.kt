...
internal fun AsmGen.translateLoadStore(insn: IRInstruction) {
    ...
    when (insn.opcode) {
        Opcode.LOAD -> {
            ...
            when {
                value != null -> emitLine("move$s  #${value.and(0xffff)}, ${regAddr(dst)}")
                sym != null -> {
...
private fun AsmGen.translateFloatLoadStore(insn: IRInstruction, target: String) {
    ...
    when (insn.opcode) {
        ...
        Opcode.LOADM -> {
            emitLine("fmove.d  $target, ${fpuRegName(fpReg1)}")
        }

        Opcode.LOADR -> {
            val src = fpReg2 ?: error("LOADR.f needs fpReg2")
            emitLine("fmove.d  ${fpuRegName(src)}, ${fpuRegName(fpReg1)}")
        }
        ...
        Opcode.STOREM -> {
            emitLine("fmove.d  ${fpuRegName(fpReg1)}, $target")
        }
...
private fun AsmGen.emitFloadConstant(fpReg: RegisterNum, value: Double) {
    when {
        ...
        value == 1.0 -> emitLine("fmovecr  #\$0e, ${fpuRegName(fpReg)}")
            ...
            emitLine("fmove.d  (a0), ${fpuRegName(fpReg)}")
        }
    }
}
