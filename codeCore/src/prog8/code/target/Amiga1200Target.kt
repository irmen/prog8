...
class Amiga1200Target: ICompilationTarget,
    ...
    companion object {
        const val NAME = "amiga1200"

        /** needed to map bank numbers to libraries in extsub/CALLFAR library calls */
        val LibraryNumbers = Amiga500Target.LibraryNumbers
    }
    ...
    override fun launchEmulator(selectedEmulator: Int, programNameWithPath: Path, quiet: Boolean) {
        if(selectedEmulator!=1) {
            System.err.println("The amiga1200 target only supports the main emulator (Amitools's vamos).")
            return
        }
