...
private fun runFixpointOptimizations(program: PtProgram, ctx: OptimizerContext) {
    ...
    while (ctx.errors.noErrors() &&
        ...
        + MemoryOptimizers.optimizeLsbMsbOnStructfields(program, ctx.options)
        + ComparisonOptimizers.optimizeFloatComparesToZero(program, ctx.options)
        ...
        + ExpressionOptimizers.optimizeOperandOrder(program, ctx.options)
